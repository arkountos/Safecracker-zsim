...
class Scheduler : public GlobAlloc, public Callee {
    ...
    public:
        ...
        uint32_t getScheduledGid(uint32_t cid) const { return (contexts[cid].state == USED)? contexts[cid].curThread->gid : (uint32_t)-1; }
        uint32_t getScheduledTid(uint32_t cid) const { return (contexts[cid].state == USED)? getTid(contexts[cid].curThread->gid) : (uint32_t)-1; }

        uint32_t getCurrentCid(uint32_t gid) const {
            g_unordered_map<uint32_t, ThreadInfo*>::const_iterator it = gidMap.find(gid);
            assert((it != gidMap.end()));
            ThreadInfo* th = it->second;
            uint32_t cid = th->cid;
            if (cid == (uint32_t)-1) return cid;
            // If cid is valid, we still need to check it's not outdated
            return contexts[cid].curThread == th? cid : (uint32_t)-1;
        }

        uint64_t getCurCoreCycle(uint32_t gid) const {
            g_unordered_map<uint32_t, ThreadInfo*>::const_iterator it = gidMap.find(gid);
            assert((it != gidMap.end()));
            ThreadInfo* th = it->second;
            uint32_t cid = th->cid;
            if (cid == (uint32_t)-1) panic("GetCurCoreCycle() called when thread is descheduled!")
            // If cid is valid, we still need to check it's not outdated
            return zinfo->cores[cid]->getCycles();
        }
