...
SetAssocArray::SetAssocArray(uint32_t _numLines, uint32_t _assoc, HashFamily* _hf) : rp(0), hf(_hf), numLines(_numLines), assoc(_assoc)  {
...
#ifdef CLU_STATS_ENABLED
    if (fullyInvalidate) {
        // TODO: EITHER not a cache scrubbing instruction
        // OR it is a cache scrubbing instruction and retId is not -1
        //TODO: fix this properly for the case of cache scrubbing!!!
        //if (!isCacheScrubbingReq || (isCacheScrubbingReq && (retId != (unsigned)-1))) {
        if (retId != (unsigned) -1) {
...
void SetAssocArray::clear() {
    for (uint32_t cand = 0; cand < numLines; cand++) {
        rp->replaced(cand);
        array[cand] = 0;
    }
}
...
ZArray::ZArray(uint32_t _numLines, uint32_t _ways, uint32_t _candidates, HashFamily* _hf) //(int _size, int _lineSize, int _assoc, int _zassoc, ReplacementPolicy<T>* _rp, int _hashType)
    : rp(0), hf(_hf), numLines(_numLines), ways(_ways), cands(_candidates)
{
