...
class TimingCore : public Core {
    ...
    private:
        inline void loadAndRecord(Address addr, MASize_t size, Address base, uint32_t baseReg);
        inline void storeAndRecord(Address addr, MASize_t size, Address base, uint32_t baseReg);
        ...
        static void LoadAndRecordFunc(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, REG baseReg);
        static void StoreAndRecordFunc(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, REG baseReg);
        static void BblAndRecordFunc(THREADID tid, ADDRINT bblAddr, BblInfo* bblInfo);
        static void PredLoadAndRecordFunc(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, BOOL pred, REG baseReg);
        static void PredStoreAndRecordFunc(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, BOOL pred, REG baseReg);
