...
class Cache : public BaseCache {
    protected:
        ...
        uint32_t tagLat; //latency of an tagLookup

        g_string name;

    public:
        Cache(CC* _cc, CacheArray* _array, ReplPolicy* _rp, uint32_t _accLat, uint32_t _invLat,
                uint32_t _tagLat, const g_string& _name);
    ...
    protected:
     ...
     public:
        virtual uint64_t scrubInvalidate(MemReq& req);
        virtual uint64_t zeroAlloc(MemReq& req);
};
