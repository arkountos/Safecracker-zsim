...
uint64_t MD1Memory::access(MemReq& req) {
...
#endif
#endif // MAXSIM_ENABLED
    assert(!(IsZeroAlloc(req.flags)));
    assert(!((req.flags & MemReq::CACHE_SCRUB_2)||(req.flags & MemReq::CACHE_SCRUB_3)));
    LcpVirtualMemory* lcp = dynamic_cast<LcpVirtualMemory*>(zinfo->vm);
    uint64_t extraLat = 0;
    if (lcp != nullptr && req.type == PUTX) {
        extraLat = lcp->dirtyWriteBack(req.lineAddr);
    }
    switch (req.type) {
        case PUTX:
            //Dirty wback
        ...
        case GETS:
            ...
            *req.state = req.is(MemReq::NOEXCL)? S : E;
            break;
        case GETX:
            if (req.flags & MemReq::ZERO_ALLOC) {
                profZeroAllocReads.atomicInc();
                profTotalZeroAllocRdLat.atomicInc(curLatency);
            } else {
                profReads.atomicInc();
                profTotalRdLat.atomicInc(curLatency);
            }
    ...
    return req.cycle + ((req.type == PUTS)? 0 /*PUTS is not a real access*/ : curLatency) + extraLat;
}
