...
TimingCache::TimingCache(CC* _cc, CacheArray* _array, ReplPolicy* _rp,
    ...
    : Cache(_cc, _array, _rp, _accLat, _invLat, _tagLat, _name), numMSHRs(mshrs), tagLat(_tagLat), ways(_ways), cands(_cands)
{
    lastFreeCycle = 0;
    lastAccCycle = 0;
    assert(numMSHRs > 0);
