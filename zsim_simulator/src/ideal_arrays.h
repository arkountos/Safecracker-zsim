...
class IdealLRUArray : public CacheArray {
    ...
    public:
        ...
        void init(ReplPolicy* _rp) { assert(_rp == rp); }
};
...
class IdealLRUPartArray : public CacheArray {
    ...
    public:
        IdealLRUPartArray(uint32_t _numLines) : rp(0), numLines(_numLines) {
        ...
        void clear() {panic("Not implemented");}
        uint32_t getNumLines() const { return numLines; }
        void init(ReplPolicy* _rp) {
            IdealLRUPartReplPolicy* irp = dynamic_cast<IdealLRUPartReplPolicy*>(_rp);
