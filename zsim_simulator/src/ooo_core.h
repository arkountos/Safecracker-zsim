...
#define LOAD_STORE_RECORD_SIZE 1024
...
class OOOCore : public Core {
    private:
        ...
        Address loadAddrs[LOAD_STORE_RECORD_SIZE];
        Address storeAddrs[LOAD_STORE_RECORD_SIZE];
        bool loadInZeroAlloc[LOAD_STORE_RECORD_SIZE];
        bool storeInZeroAlloc[LOAD_STORE_RECORD_SIZE];
        Address loadBase[LOAD_STORE_RECORD_SIZE];
        Address storeBase[LOAD_STORE_RECORD_SIZE];
        uint32_t loadBaseReg[LOAD_STORE_RECORD_SIZE];
        uint32_t storeBaseReg[LOAD_STORE_RECORD_SIZE];

        //record regWrites
        static const int REG_ARG_NUM_MAX = LOAD_STORE_RECORD_SIZE * 5;
        bool maIsRegWrite[REG_ARG_NUM_MAX];
        uint32_t maWrittenReg[REG_ARG_NUM_MAX];
        uint64_t maWrittenRegBaseValue[REG_ARG_NUM_MAX];
        uint64_t maWrittenRegOffset[REG_ARG_NUM_MAX];
        uint32_t maWrittenRegSourceReg[REG_ARG_NUM_MAX];
        bool maWrittenRegIsClear[REG_ARG_NUM_MAX];
        int regMaNum;

        uint32_t loads;
        uint32_t stores;
#ifdef CLU_STATS_ENABLED
        MASize_t loadSizes[LOAD_STORE_RECORD_SIZE];
        MASize_t storeSizes[LOAD_STORE_RECORD_SIZE];
#endif
#ifdef POINTER_TAGGING_ENABLED
        PointerTag_t loadTag[LOAD_STORE_RECORD_SIZE];
        PointerTag_t storeTag[LOAD_STORE_RECORD_SIZE];
#endif
#ifdef MA_PROF_ENABLED
        int32_t loadOffset[LOAD_STORE_RECORD_SIZE];
        int32_t storeOffset[LOAD_STORE_RECORD_SIZE];
#endif
        uint64_t lastStoreCommitCycle;
        uint64_t lastStoreAddrCommitCycle; //tracks last store addr uop, all loads queue behind it
        uint64_t maxCommitCycle;
        ...
        ReorderBuffer<8, 2> loadQueue;
        ReorderBuffer<8, 2> storeQueue;
        WindowStructure<1024, 16 /*size*/> insWindow; //NOTE: IW width is implicitly determined by the decoder, which sets the port masks according to uop type
        ReorderBuffer<32, 2> rob;
        // Note: For KNL core, changed to 1024 B/PHT entries, 10-bit history --> < 2 KBytes
        BranchPredictorPAg<10, 10, 10> branchPred;
        CycleQueue<8> uopQueue;  // models issue queue
        ...
        WindowStructure<1536, 32 /*size*/> insWindow; 
        ReorderBuffer<192, 4> rob;
        BranchPredictorPAg<11, 18, 14> branchPred;
        CycleQueue<56> uopQueue;  // models issue queue

#elif defined(SKYLAKE)
        ReorderBuffer<72, 4> loadQueue;
        ReorderBuffer<56, 4> storeQueue;
        WindowStructure<2048, 36 /*size*/> insWindow; 
        ReorderBuffer<224, 4> rob;
        BranchPredictorPAg<12, 19, 15> branchPred;
        CycleQueue<64> uopQueue;  // models issue queue

#else
        static_assert(false, "Need a core model");
#endif
        ...
        uint64_t zeroAllocInstrs, zeroAllocUops, zeroAllocBranchUops, zeroAllocFpAddSubUops, zeroAllocFpMulDivUops, zeroAllocBbls, zeroAllocApproxInstrs;

#ifdef OOO_STALL_STATS
        Counter profFetchStalls, profDecodeStalls, profIssueStalls;
#endif
        Counter profErrOOOLoads, profErrOOOStores;
    ...
    public:
        ...
        //cache scrub
        void zeroAllocRegion(Address base, uint32_t numWords, uint32_t zeroAllocLevel, MASize_t size, THREADID tid);
    ...
    private:
        inline void load(Address addr, uint32_t size, Address base, bool inZeroAlloc, uint32_t baseReg);
        inline void store(Address addr, uint32_t size, Address base, bool inZeroAlloc, uint32_t baseReg);
        ...
        inline void predFalseMemOp(bool inZeroAlloc);

        inline void branch(Address pc, bool taken, Address takenNpc, Address notTakenNpc);

        inline void bbl(THREADID tid, Address bblAddr, BblInfo* bblInfo, bool inZeroAlloc);
