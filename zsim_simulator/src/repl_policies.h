...
class ReplPolicy : public GlobAlloc {
    ...
    public:
        ...
        virtual uint32_t rankCands(const MemReq* req, const compressed::Cands& cands) = 0;
...
#define DECL_RANK_BINDINGS DECL_RANK_BINDING(SetAssocCands); DECL_RANK_BINDING(ZCands); DECL_RANK_BINDING(const compressed::Cands&);
...
class LRUReplPolicy : public ReplPolicy {
    ...
    public:
        ...
        void update(uint32_t id, const MemReq* req) {
            array[id] = timestamp++;
            //info("  LRU: lineId %d timestamp %lu", id, array[id]);
        }
    ...
    private:
        inline uint64_t score(uint32_t id) { //higher is least evictable
            ...
            return (sharersAware? cc->numSharers(id) : 0)*timestamp + array[id];
        }
};
