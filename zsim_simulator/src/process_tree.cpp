...
void DumpEventualStats(uint32_t procIdx, const char* reason
    ...
    if (maxineVMOperationMode != MAXINE_VM_OPERATION_MODE_SAME) {
        ...
        //poan: only increase procEventualDumps when this is not GC related
        zinfo->procEventualDumps++;
    }
#endif
    info("Dumping eventual stats ID %ld for process GROUP %d (%s)", zinfo->procEventualDumps, p, reason);
    if (zinfo->procEventualDumps == zinfo->maxProcEventualDumps && zinfo->maxProcEventualDumps != 0) {
...
static void PopulateLevel(Config& config, const std::string& prefix, std::vector<ProcessTreeNode*>& globProcVector, ProcessTreeNode* parent, uint32_t& procIdx, uint32_t& groupIdx) {
    ...
    while (true) {
        ...
        g_vector<bool> mask;
        g_vector<bool> nullMask;

        if (!zinfo->traceDriven) {
            mask = ParseMask(config.get<const char*>(p_ss.str() +  ".mask", DefaultMaskStr().c_str()), zinfo->numCores);
            nullMask = ParseMask(config.get<const char*>(p_ss.str() +  ".nullMask", DefaultMaskStr().c_str()), zinfo->numCores);
        ...
        ProcessTreeNode* ptn = new ProcessTreeNode(procIdx, groupIdx, startFastForwarded, startPaused, syncedFastForward, clockDomain, portDomain, dumpHeartbeats, dumpsResetHeartbeats, restarts, mask, nullMask, ffiPoints, syscallBlacklistRegex, gpr);
...
void CreateProcessTree(Config& config) {
    ProcessTreeNode* rootNode = new ProcessTreeNode(-1, -1, false, false, SFF_NEVER, 0, 0, 0, false, 0, g_vector<bool> {}, g_vector<bool> {}, g_vector<uint64_t> {}, g_string {}, nullptr);
    uint32_t procIdx = 0;
    uint32_t groupIdx = 0;
