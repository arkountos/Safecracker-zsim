...
//VOID HandleMagicOp(THREADID tid, ADDRINT* op, ADDRINT arg1,  ADDRINT arg2, CONTEXT* ctxt);
VOID HandleMagicOp(THREADID tid, ADDRINT* op, ADDRINT arg1,  ADDRINT arg2);
...
VOID recordStackPointers(THREADID tid, ADDRINT rbp, ADDRINT rsp);
...
VOID PIN_FAST_ANALYSIS_CALL IndirectLoadSingle(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, REG baseReg) {
    fPtrs[tid].loadPtr(tid, addr, size, base, baseReg);
}

VOID PIN_FAST_ANALYSIS_CALL IndirectStoreSingle(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, REG baseReg) {
    fPtrs[tid].storePtr(tid, addr, size, base, baseReg);
}

VOID PIN_FAST_ANALYSIS_CALL AfterStoreSingle(THREADID tid) {
    fPtrs[tid].afterStorePtr(tid);
}
...
VOID PIN_FAST_ANALYSIS_CALL IndirectPredLoadSingle(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, BOOL pred, REG baseReg) {
    fPtrs[tid].predLoadPtr(tid, addr, size, base, pred, baseReg);
}

VOID PIN_FAST_ANALYSIS_CALL IndirectPredStoreSingle(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, BOOL pred, REG baseReg) {
    fPtrs[tid].predStorePtr(tid, addr, size, base, pred, baseReg);
}

VOID PIN_FAST_ANALYSIS_CALL RegisterWrite(THREADID tid,
                                          REG destReg,
                                          ADDRINT valueInBaseReg, ADDRINT valueInIndexReg,
                                          UINT32 scale, INT64 disp,
                                          BOOL isWrite,
                                          ADDRINT effectiveAddress,
                                          UINT32 regWriteType
                                          ) {
    assert(!isWrite);
    int64_t offset = valueInIndexReg * scale + disp;
    Address calculatedEA = valueInBaseReg + offset;
    if (effectiveAddress != calculatedEA) {
        //info("[WARNING: RegisterWrite mismatch!] effectiveAddr %lx calculatedEA %lx", effectiveAddress, calculatedEA);
        offset = effectiveAddress - valueInBaseReg;
    }
    //profiling type of memory read
    if (!procTreeNode->isInFastForward()) {
        ...
        case 3:
            ...
            break;
        case 4:
            zinfo->numNoRegMemRead++;
            break;
        default:
            panic("Only have 4 types of register writes, received %u !", regWriteType);
        }
    }
    fPtrs[tid].regWritePtr(tid, destReg, valueInBaseReg, offset, REG_INVALID(), false /*isClear*/);
}

VOID PIN_FAST_ANALYSIS_CALL RegisterWriteClear(THREADID tid, REG destReg, BOOL isXchgMagicOp) {
    if (!procTreeNode->isInFastForward()) {
        if (isXchgMagicOp) {
            zinfo->numXchgMagicOp++;
        } else {
            zinfo->numClearReg++;
        }
    }
    fPtrs[tid].regWritePtr(tid, destReg, 0, 0, REG_INVALID(), true/*isClear*/);
}

VOID PIN_FAST_ANALYSIS_CALL RegisterWriteRegToReg(THREADID tid, REG destReg, REG sourceReg) {
    assert(sourceReg != REG_INVALID());
    if (!procTreeNode->isInFastForward()) {
        zinfo->numRegToReg++;
    }
    fPtrs[tid].regWritePtr(tid, destReg, 0, 0, sourceReg, false/*isClear*/);
}
...
VOID JoinAndLoadSingle(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, REG baseReg) {
    Join(tid);
    fPtrs[tid].loadPtr(tid, addr, size, base, baseReg);
}

VOID JoinAndStoreSingle(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, REG baseReg) {
    Join(tid);
    fPtrs[tid].storePtr(tid, addr, size, base, baseReg);
}
...
VOID JoinAndPredLoadSingle(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, BOOL pred, REG baseReg) {
    Join(tid);
    fPtrs[tid].predLoadPtr(tid, addr, size, base, pred, baseReg);
}

VOID JoinAndPredStoreSingle(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, BOOL pred, REG baseReg) {
    Join(tid);
    fPtrs[tid].predStorePtr(tid, addr, size, base, pred, baseReg);
}

VOID JoinRegWrite(THREADID tid, REG destReg, ADDRINT valueInBaseReg, INT64 offset, REG sourceReg, BOOL isClear) {
    Join(tid);
    info("^^^^^^ tid %i destReg %i value %lx", tid, destReg, valueInBaseReg);
    fPtrs[tid].regWritePtr(tid, destReg, valueInBaseReg, offset, sourceReg, isClear);
}

// NOP variants: Do nothing
VOID NOPLoadStoreSingle(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, REG baseReg) {}
...
VOID NOPPredLoadStoreSingle(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, BOOL pred, REG baseReg) {}
VOID NOPAfterStoreSingle(THREADID tid) {}
VOID NOPRegWrite(THREADID tid, REG destReg, ADDRINT valueInBaseReg, INT64 offset, REG sourceReg, BOOL isClear){}
...
// Called by a magic op to leave FF
VOID FFIExternalFFExit() {
    assert(ffiEnabled);
    assert(!ffiNFF);
    // NOTE: We change done -> limit instead of limit -> done to avoid any
    // issues on future limits (e.g., if future limit is not computed using
    // current one; IDK at this point). This safely results in the FFI
    // machinery ending the current FF period in the next basic block.
    info("FFI: Forcing FF exit at %ld instrs", ffiInstrsDone);
    ffiInstrsDone = ffiInstrsLimit;
}
...
*/
static const InstrFuncPtrs joinPtrs = {JoinAndLoadSingle, JoinAndStoreSingle, JoinAndBasicBlock, JoinAndRecordBranch, JoinAndPredLoadSingle, JoinAndPredStoreSingle, NOPAfterStoreSingle, FPTR_JOIN, JoinRegWrite};
static const InstrFuncPtrs nopPtrs = {NOPLoadStoreSingle, NOPLoadStoreSingle, NOPBasicBlock, NOPRecordBranch, NOPPredLoadStoreSingle, NOPPredLoadStoreSingle, NOPAfterStoreSingle, FPTR_NOP, NOPRegWrite};
static const InstrFuncPtrs retryPtrs = {NOPLoadStoreSingle, NOPLoadStoreSingle, NOPBasicBlock, NOPRecordBranch, NOPPredLoadStoreSingle, NOPPredLoadStoreSingle, NOPAfterStoreSingle, FPTR_RETRY, NOPRegWrite};
static const InstrFuncPtrs ffPtrs = {NOPLoadStoreSingle, NOPLoadStoreSingle, FFBasicBlock, NOPRecordBranch, NOPPredLoadStoreSingle, NOPPredLoadStoreSingle, NOPAfterStoreSingle, FPTR_NOP, NOPRegWrite};

static const InstrFuncPtrs ffiPtrs = {NOPLoadStoreSingle, NOPLoadStoreSingle, FFIBasicBlock, NOPRecordBranch, NOPPredLoadStoreSingle, NOPPredLoadStoreSingle, NOPAfterStoreSingle, FPTR_NOP, NOPRegWrite};
static const InstrFuncPtrs ffiEntryPtrs = {NOPLoadStoreSingle, NOPLoadStoreSingle, FFIEntryBasicBlock, NOPRecordBranch, NOPPredLoadStoreSingle, NOPPredLoadStoreSingle, NOPAfterStoreSingle, FPTR_NOP, NOPRegWrite};
...
void EnterFastForward() {
    ...
    info("process %d entering FF", procIdx);
...
void ExitFastForward() {
    ...
    info("process %d exiting FF", procIdx);
...
void SystemExitFastForward() {
    ...
    for (uint32_t p = 0; p < zinfo->numProcs; p++){
        if(zinfo->procArray[p]->isInFastForward()) {
            info("Process %d in FF, exiting FF", p);
            zinfo->procArray[p]->exitFastForward();
        } else {
            info("Process %d not in FF, skip", p);
        }
    }
    __sync_synchronize(); //make change globally visible
...
void MemoryAccessInstrument(INS ins, AFUNPTR RegWriteFuncPtr) {
    if (zinfo->debugPointerRewrite) {
        info(" >>>>> %s   INS_OperandCount(ins) %i Opcode: %i", INS_Disassemble(ins).c_str(), INS_OperandCount(ins), INS_Opcode(ins));
    }
    //total number of possible regs is REG_LAST = 411 for this run
    UINT32 count = 0;
    UINT32 memCt = 0;
    REG destReg = REG_INVALID();
    UINT32 opCount = INS_OperandCount(ins);
    UINT32 memOpIdx = opCount;
    for (UINT32 opIdx = 0; opIdx < opCount; opIdx++) {
        ...
        if ((INS_OperandIsReg(ins, opIdx)) && (INS_OperandWritten(ins, opIdx))) {
            // count # of destination register
            REG _reg = INS_OperandReg(ins, opIdx) ;
            if ((_reg == REG_RFLAGS) || (_reg == REG_INVALID())) {
                // skipping these registers
            } else {
                count++;
                destReg = _reg;
            }
    ...
    assert_msg(count == 1, "expected one destination reg got %i INS %s ", count, INS_Disassemble(ins).c_str());
    assert_msg(memCt == 1, "expected one memory source got %i INS %s Opcode: %u", memCt, INS_Disassemble(ins).c_str(), INS_Opcode(ins));
    assert(memOpIdx < opCount);
    //assert(baseReg != REG_INVALID());
    assert(destReg != REG_INVALID());

    REG baseReg = INS_OperandMemoryBaseReg(ins, memOpIdx);
    REG indexReg = INS_OperandMemoryIndexReg(ins, memOpIdx);
    UINT32 scale = INS_OperandMemoryScale(ins, memOpIdx);
    INT64 disp = INS_OperandMemoryDisplacement(ins, memOpIdx);
    BOOL isWrite = INS_OperandWritten(ins, memOpIdx);
    if (indexReg == REG_INVALID() && baseReg != REG_INVALID()) {
        if (zinfo->debugPointerRewrite) {
            info(" [!@#$] ins %s baseReg %i name %s scale %u disp %lx isWrite %i",
                 INS_Disassemble(ins).c_str(), baseReg, REG_StringShort(baseReg).c_str(), scale, disp, isWrite);
        }
    }

    if (!isWrite) {
        // we only care about reads, memWrites shouldn't directly change the value of the base reg used later, so ignore them
        if (baseReg != REG_INVALID() && indexReg != REG_INVALID()) {
            INS_InsertCall(ins, IPOINT_BEFORE, RegWriteFuncPtr, IARG_FAST_ANALYSIS_CALL,
                           IARG_THREAD_ID,
                           IARG_UINT32, destReg,
                           IARG_REG_VALUE, baseReg, IARG_REG_VALUE, indexReg,
                           IARG_UINT32, scale, IARG_ADDRINT, disp,
                           IARG_BOOL, isWrite,
                           IARG_MEMORYREAD_EA,
                           IARG_UINT32, 1,
                           IARG_END);
        } else if (baseReg != REG_INVALID()) {
            assert(indexReg == REG_INVALID());
            INS_InsertCall(ins, IPOINT_BEFORE, RegWriteFuncPtr, IARG_FAST_ANALYSIS_CALL,
                           IARG_THREAD_ID,
                           IARG_UINT32, destReg,
                           IARG_REG_VALUE, baseReg, IARG_ADDRINT, 0L,
                           IARG_UINT32, scale, IARG_ADDRINT, disp,
                           IARG_BOOL, isWrite,
                           IARG_MEMORYREAD_EA,
                           IARG_UINT32, 2,
            ...
            if (zinfo->debugPointerRewrite) { info("[!!!!] ins %s", INS_Disassemble(ins).c_str());}
            INS_InsertCall(ins, IPOINT_BEFORE, RegWriteFuncPtr, IARG_FAST_ANALYSIS_CALL,
                           IARG_THREAD_ID,
                           IARG_UINT32, destReg,
                           IARG_ADDRINT, 0L, IARG_ADDRINT, 0L,
                           IARG_UINT32, scale, IARG_ADDRINT, disp,
                           IARG_BOOL, isWrite,
                           IARG_MEMORYREAD_EA,
                           IARG_UINT32, 4,
            ...
            assert(baseReg == REG_INVALID());
            if (zinfo->debugPointerRewrite) {
                info("[@@@@] ins %s indexReg %i name %s",
                     INS_Disassemble(ins).c_str(), indexReg, REG_StringShort(indexReg).c_str());
            }
            INS_InsertCall(ins, IPOINT_BEFORE, RegWriteFuncPtr, IARG_FAST_ANALYSIS_CALL,
                           IARG_THREAD_ID,
                           IARG_UINT32, destReg,
                           IARG_ADDRINT, 0L, IARG_REG_VALUE, indexReg,
                           IARG_UINT32, scale, IARG_ADDRINT, disp,
                           IARG_BOOL, isWrite,
                           IARG_MEMORYREAD_EA,
                           IARG_UINT32, 3,
...
uint32_t GetAllRegInfo(INS ins, g_vector<REG> &destRegs, g_vector<REG> &srcRegs, g_vector<REG> &memRegs) {
    uint32_t immCount = 0;
    for (UINT32 opIdx = 0; opIdx < INS_OperandCount(ins); opIdx++) {
        ...
        if( INS_OperandMemoryBaseReg(ins, opIdx) != REG_INVALID()) {
            memRegs.push_back(INS_OperandMemoryBaseReg(ins, opIdx));
        }
        ...
        if (INS_OperandIsReg(ins, opIdx)) {
            if (INS_OperandWritten(ins, opIdx)) {
                ...
                if ((_reg == REG_RFLAGS) || (_reg == REG_INVALID())) {
                    ...
                    destRegs.push_back(_reg);
                }
            }
            // a register can be both the source and operand
            if (INS_OperandRead(ins, opIdx)) {
                REG _reg = INS_OperandReg(ins, opIdx) ;
                if ((_reg == REG_RFLAGS) ||  (_reg == REG_INVALID())) {
                    // skipping these registers
                } else {
                    srcRegs.push_back(_reg);
                }
            }
        }
    }
...
void DebugGetAllRegOfIns(INS ins, g_vector<REG> &destRegs, g_vector<REG> &debugRegs) {
    info(" >>>>> %s   INS_OperandCount(ins) %i Opcode: %i", INS_Disassemble(ins).c_str(), INS_OperandCount(ins), INS_Opcode(ins));
    UINT32 count = 0;
    UINT32 memCt = 0;
    UINT32 srcCt = 0;
    for (UINT32 opIdx = 0; opIdx < INS_OperandCount(ins); opIdx++) {
        if( INS_OperandMemoryBaseReg(ins, opIdx) != REG_INVALID()) {
            debugRegs.push_back(INS_OperandMemoryBaseReg(ins, opIdx));
        }
        BOOL isMem = INS_OperandIsMemory(ins, opIdx);
        BOOL isSource = INS_OperandRead(ins, opIdx);
        BOOL isReg = INS_OperandIsReg(ins, opIdx);
        if (isSource && isReg) { srcCt++; }
        if (isMem) {
            memCt++;
            REG baseR = INS_OperandMemoryBaseReg(ins, opIdx);
            REG indexR = INS_OperandMemoryIndexReg(ins, opIdx);
            ADDRDELTA displacement = INS_MemoryDisplacement(ins);
            UINT32 scale = INS_MemoryScale(ins);
            info("operand ind %i isreg %i isImplicit %i isMem %i "
                 " isImmediate %i isWritten %i isRead %i "
                 " isWrittenOnly %i isReadOnly %i "
                 " reg#:%i name: %s BaseReg: %i name: %s"
                 " IndexRex: %i name :%s"
                 " displacement %lx scale: %i",
                 opIdx, isReg, INS_OperandIsImplicit(ins, opIdx),
                 isMem, INS_OperandIsImmediate(ins, opIdx),
                 INS_OperandWritten(ins, opIdx), isSource,
                 INS_OperandWrittenOnly(ins, opIdx), INS_OperandReadOnly(ins, opIdx),
                 INS_OperandReg(ins, opIdx),
                 REG_StringShort(INS_OperandReg(ins, opIdx)).c_str(),
                 baseR, REG_StringShort(baseR).c_str(),
                 indexR, REG_StringShort(indexR).c_str(),
                 displacement,
                 scale);
        } else {
            REG memBaseReg = INS_MemoryBaseReg(ins);
            REG operandMemBaseReg = INS_OperandMemoryBaseReg(ins, opIdx);
            info("operand ind %i isreg %i isImplicit %i isMem %i "
                 " isImmediate %i isWritten %i isRead %i "
                 " isWrittenOnly %i isReadOnly %i "
                 " reg#:%i name: %s memBaseReg %i name: %s operandMemBaseReg %i name: %s",
                 opIdx, isReg, INS_OperandIsImplicit(ins, opIdx),
                 isMem, INS_OperandIsImmediate(ins, opIdx),
                 INS_OperandWritten(ins, opIdx), isSource,
                 INS_OperandWrittenOnly(ins, opIdx), INS_OperandReadOnly(ins, opIdx),
                 INS_OperandReg(ins, opIdx),
                 REG_StringShort(INS_OperandReg(ins, opIdx)).c_str(),
                 memBaseReg, REG_StringShort(memBaseReg).c_str(),
                 operandMemBaseReg, REG_StringShort(operandMemBaseReg).c_str()
                 );
        }
        if ((INS_OperandIsReg(ins, opIdx)) && (INS_OperandWritten(ins, opIdx))) {
            // count # of destination register
    ...
    if (memCt > 1) {
        assert((INS_Opcode(ins) == 74) || (count <= 1) || (INS_IsCall(ins)));
    }
    return ;
}
...
VOID Instruction(INS ins) {
    ...
    //poan: record RSP if the stack is modified
    if (INS_IsStackWrite(ins)) {
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR) recordStackPointers, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                       IARG_REG_VALUE, REG_EBP, IARG_REG_VALUE, REG_ESP, IARG_END);
    }
    ...
    if (!procTreeNode->isInFastForward() || !zinfo->ffReinstrument) {
        ...
        AFUNPTR AfterStoreFuncPtr = (AFUNPTR) AfterStoreSingle;
            ...
            if (!INS_IsPredicated(ins)) {
                if (hasBaseReg) {
                    INS_InsertCall(ins, IPOINT_BEFORE, LoadFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                   IARG_MEMORYREAD_EA, IARG_MEMORYREAD_SIZE, IARG_REG_VALUE, baseReg,
                                   IARG_UINT32, baseReg, IARG_END);
                } else {
                    INS_InsertCall(ins, IPOINT_BEFORE, LoadFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                   IARG_MEMORYREAD_EA, IARG_MEMORYREAD_SIZE, IARG_MEMORYREAD_EA,
        ...
        if (INS_HasMemoryRead2(ins)) {
            ...
            if (!INS_IsPredicated(ins)) {
                if (hasBaseReg) {
                    INS_InsertCall(ins, IPOINT_BEFORE, LoadFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                   IARG_MEMORYREAD2_EA, IARG_MEMORYREAD_SIZE, IARG_REG_VALUE, baseReg,
                                   IARG_UINT32, baseReg, IARG_END);
                } else {
                    INS_InsertCall(ins, IPOINT_BEFORE, LoadFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                   IARG_MEMORYREAD2_EA, IARG_MEMORYREAD_SIZE, IARG_MEMORYREAD2_EA,
                                   IARG_UINT32, 0, IARG_END);
                }
            } else {
                if (hasBaseReg) {
            ...
            if (!INS_IsPredicated(ins)) {
                if (hasBaseReg) {
                    INS_InsertCall(ins, IPOINT_BEFORE, StoreFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                   IARG_MEMORYWRITE_EA, IARG_MEMORYWRITE_SIZE, IARG_REG_VALUE, baseReg,
                                   IARG_UINT32, baseReg, IARG_END);
                    //INS_InsertCall(ins, IPOINT_AFTER, AfterStoreFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_END);
                } else {
                    INS_InsertCall(ins, IPOINT_BEFORE, StoreFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                   IARG_MEMORYWRITE_EA, IARG_MEMORYWRITE_SIZE, IARG_MEMORYWRITE_EA,
                                   IARG_UINT32, 0, IARG_END);
                    //INS_InsertCall(ins, IPOINT_AFTER, AfterStoreFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_END);
                }
            } else {
                if (hasBaseReg) {
                    INS_InsertCall(ins, IPOINT_BEFORE, PredStoreFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                   IARG_MEMORYWRITE_EA, IARG_MEMORYWRITE_SIZE, IARG_REG_VALUE, baseReg,
                                   IARG_UINT32, baseReg, IARG_EXECUTING, IARG_END);
                    INS_InsertCall(ins, IPOINT_AFTER, AfterStoreFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_END);
                } else {
                    INS_InsertCall(ins, IPOINT_BEFORE, PredStoreFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID,
                                   IARG_MEMORYWRITE_EA, IARG_MEMORYWRITE_SIZE, IARG_MEMORYWRITE_EA, IARG_EXECUTING,
                                   IARG_UINT32, 0, IARG_END);
                    INS_InsertCall(ins, IPOINT_AFTER, AfterStoreFuncPtr, IARG_FAST_ANALYSIS_CALL, IARG_THREAD_ID, IARG_END);
                }
            }
    ...
    if (INS_IsXchg(ins) && INS_OperandReg(ins, 0) == REG_RCX && INS_OperandReg(ins, 1) == REG_RCX) {
        ...
        INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR) HandleMagicOp, IARG_THREAD_ID, IARG_REG_REFERENCE, REG_RCX,
            //IARG_REG_VALUE, REG_RBX, IARG_REG_VALUE, REG_RDX, IARG_CONTEXT, IARG_RETURN_REGS, REG_EAX, IARG_END);
            IARG_REG_VALUE, REG_RBX, IARG_REG_VALUE, REG_RDX, IARG_END);
    }
...
VOID recordStackPointers(THREADID tid, ADDRINT rbp, ADDRINT rsp) {
    zinfo->curRBP[(procIdx << 8) + tid] = rbp;
    zinfo->curRSP[(procIdx << 8) + tid] = rsp;
}
...
void SimThreadStart(THREADID tid) {
    ...
    if (procTreeNode->isNullThread(tid)) {
        info("  Thread %d is a nullThread map to null mask", tid);
        zinfo->sched->start(procIdx, tid, procTreeNode->getNullMask());
    } else {
...
#define ZSIM_MAGIC_OP_FFI_FF_EXIT       (1050)

#define ZSIM_MAGIC_OP_REGISTER_CLIENT_START (3001)
#define ZSIM_MAGIC_OP_REGISTER_CLIENT_END   (3002)
...
VOID HandleMagicOp(THREADID tid, ADDRINT* op, ADDRINT arg1, ADDRINT arg2) {
    switch (*op) {
        case ZSIM_MAGIC_OP_ROI_BEGIN:
            if (!zinfo->ignoreHooks) {
                ...
                if (procTreeNode->isInFastForward()) {
                    ...
                    DumpEventualStats(procIdx, "ROI_begin() within non-FF mode, dump stats");
                }
                futex_unlock(&zinfo->ffLock);
            }
            return;
        ...
        case ZSIM_MAGIC_OP_FFI_FF_EXIT:
            if (ffiEnabled) {
                if (ffiNFF) {
                    warn("Thread %d: Ignoring FFI_FF_EXIT magic op (in FFI mode, but not fast-forwarding!)", tid);
                } else {
                    FFIExternalFFExit();
                }
            } else {
                info("Thread %d: Ignoring FFI_FF_EXIT magic op (not in FFI mode)", tid);
            }
            return;
        case ZSIM_MAGIC_OP_SYSTEM_ROI_BEGIN:
            if (!zinfo->ignoreHooks) {
                futex_lock(&zinfo->ffLock);
                info("System_ROI_BEGIN, exiting fast-forward for all processes");
                SystemExitFastForward();
                futex_unlock(&zinfo->ffLock);
            }
            return;
        ...
        default:
            ...
            if ((MAXSIM_M_OPC_LO <= (*op)) && ((*op) <= MAXSIM_M_OPC_HI)) {
                MaxSimMediator::getInst().HandleMaxSimMagicOp(tid, op, arg1, arg2);
                break;
            }
#endif
...
VOID FakeRDTSCPost(THREADID tid, REG* eax, REG* edx) {
    ...
    //info("VirtGetPhaseRDTSC: %lu", curCycle);
    if (cid < zinfo->numCores) {
        curCycle += zinfo->cores[cid]->getPhaseCycles();
    }
    //info("VirtGetPhaseRDTSC + getPhaseCycles: %lu", curCycle);
