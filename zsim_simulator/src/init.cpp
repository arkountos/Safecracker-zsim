...
#include "predictor_non_incl_cache.h"
...
#include "compressed_repl_policies.h"
...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name, uint32_t bankSize, bool isTerminal, uint32_t domain
    ...
    if (arrayType == "SetAssoc" || arrayType == "Compressed") {
        numHashes = 1;
    } else if (arrayType == "Z") {
        numHashes = ways;
        assert(ways > 1);
    ...
    if (arrayType == "SetAssoc") {
        array = new SetAssocArray(numLines, ways, hf);
    } else if (arrayType == "Z") {
        array = new ZArray(numLines, ways, candidates, hf);
    ...
    } else if (replType == "CAMP") {
        uint32_t M = config.get<uint32_t>(prefix + "repl.M", 3);  // default is 3 in CAMP
        uint32_t samplingFactor = config.get<uint32_t>(prefix + "repl.samplingFactor", 128);
        auto* carray = dynamic_cast<compressed::CacheArray*>(array);
        assert(carray != nullptr);
        rp = new compressed::CAMPReplPolicy(carray, numLines, M, samplingFactor);
    } else if (replType == "SRRIP" || replType == "BRRIP" || replType == "DRRIP") {
        uint32_t M = config.get<uint32_t>(prefix + "repl.M", 3);
        if (replType == "SRRIP") rp = new SRRIPReplPolicy(numLines, M);
        else if (replType == "BRRIP") rp = new BRRIPReplPolicy(numLines, M);
        else if (replType == "DRRIP") {
            bool useSetSampling = config.get<bool>(prefix + "repl.setSampling", false);
            if (useSetSampling) {
                uint32_t samplingFactor = config.get<uint32_t>(prefix + "repl.samplingFactor", 32);
                rp = new DRRIPSetSamplingReplPolicy(numLines, M, samplingFactor);
            } else {
                rp = new DRRIPReplPolicy(numLines, M);
            }
        }
    ...
    if (isTerminal) {
        ...
        if (type == "NonInclusive" || type == "PredictorNonInclusive") {
            cc = new FakeCC();
        } else {
    ...
    rp->setCC(cc);
    if (dynamic_cast<compressed::CacheArray*>(array)) {
        auto* comArray = dynamic_cast<compressed::CacheArray*>(array);
        comArray->setCC(cc);
    }

    if (!isTerminal) {
        if (type == "Simple") {
            cache = new Cache(cc, array, rp, accLat, invLat, tagLat, name);
        } else if (type == "Timing") {
            ...
            cache = new TimingCache(cc, array, rp, accLat, invLat, mshrs, tagLat, ways, timingCandidates, domain, name);
            ...
            cache = new TracingCache(cc, array, rp, accLat, invLat, tagLat, traceFile, name);
        } else if (type == "NonInclusive") {
            assert(tagLat <= accLat);
            uint32_t dataLat = accLat - tagLat;
            cache = new NonInclusiveCache(cc, array, rp, tagLat, dataLat, name);
        } else if (type == "PredictorNonInclusive") {
            assert(tagLat <= accLat);
            uint32_t dataLat = accLat - tagLat;
            auto* predictor = new MemoryAccessPredictor("predictor", true, 3, 8);
            cache = new PredictorNonInclusiveCache(cc, array, rp, tagLat, dataLat, name, predictor);
        } else {
        ...
        cache = new FilterCache(numSets, cc, array, rp, accLat, invLat, tagLat, name);
    }
...
MemObject* BuildMemoryController(Config& config, uint32_t lineSize,
        uint32_t frequency, uint32_t domain, g_string& name, string prefix) {
    //Type
    string type = config.get<const char*>(prefix + "type", "Simple");
    ...
    if (type == "Simple") {
        ...
        string dramTechIni = config.get<const char*>(prefix + "techIni");
        string dramSystemIni = config.get<const char*>(prefix + "systemIni");
        string outputDir = config.get<const char*>(prefix + "outputDir");
        string traceName = config.get<const char*>(prefix + "traceName");
...
static void InitSystem(Config& config) {
    ...
    uint32_t memControllerGroups = config.get<uint32_t>("sys.mem.controllerGroups", 1);
    g_vector<MemObject*> mems; // all memory controller in one vector
    g_vector< g_vector<MemObject*>> memGroups; // memory groups, used for stats initialization
    uint32_t totalControllers = 0;

    uint32_t memControllers = config.get<uint32_t>("sys.mem.controllers", 1);
    assert(memControllers > 0);

    //Push each group in the mems and memGroups
    for (uint32_t mg = 0; mg < memControllerGroups; mg++) {
        string prefix;
        string groupName;
        g_vector<MemObject*> ctrls;
        if (memControllerGroups > 1) {
            prefix = string("sys.mem.group") + Str(mg) + ".";
            groupName = string("group-") + Str(mg) + "-";
        } else {
            prefix = string("sys.mem.");
            groupName = "";
        }
        uint32_t memControllers = config.get<uint32_t>(prefix + "controllers", 1);
        assert(memControllers > 0);
        totalControllers += memControllers;
        for (uint32_t i = 0; i < memControllers; i++) {
            stringstream ss;
            ss << groupName << "mem-" << i;
            g_string name(ss.str().c_str());
            uint32_t domain = i*zinfo->numDomains/memControllers;
            auto* memCtrl = BuildMemoryController(config, zinfo->lineSize, zinfo->freqMHz, domain, name, prefix);
            mems.push_back(memCtrl);
            ctrls.push_back(memCtrl);
        }
        memGroups.push_back(ctrls);
    }
    ...
    if (memControllerGroups > 1) {
        for (uint32_t mg = 0; mg < memControllerGroups; mg++) {
            AggregateStat* memGroupStat = new AggregateStat(true);
            string groupName = string("memGroup-") + Str(mg);
            memGroupStat->init(gm_strdup(groupName.c_str()), "Memory controller group stats");
            for (auto mem : memGroups[mg]) mem->initStats(memGroupStat);
            zinfo->rootStat->append(memGroupStat);
        }
    } else {
...
static void InitGlobalStats() {
    ...
    zinfo->excessiveGCtid = MAX_THREADS + 1;
    zinfo->vmOpThreadtid = MAX_THREADS + 1;
    zinfo->isInVmFF = false;
    zinfo->fastForwardThreadTids = new g_vector<bool>();
    for (uint32_t _i = 0; _i < MAX_THREADS; _i++){
        zinfo->fastForwardThreadTids->push_back(false);
    }
    ...
    ProxyStat* youngGenGCStat = new ProxyStat();
    youngGenGCStat->init("youngGenGC", "# of young gen GCs", &zinfo->numYoungGenGCs);
    zinfo->rootStat->append(youngGenGCStat);

    ProxyStat* oldGenGCStat = new ProxyStat();
    oldGenGCStat->init("oldGenGC", "# of old gen GCs", &zinfo->numOldGenGCs);
    zinfo->rootStat->append(oldGenGCStat);

    ProxyStat* numDumpEvStat = new ProxyStat();
    numDumpEvStat->init("numDumpEvStat", "# of dump eventual stat requested", &zinfo->numDumpEvSt);
    zinfo->rootStat->append(numDumpEvStat);

    ProxyStat* numZeroingStat = new ProxyStat();
    numZeroingStat->init("numZeroingAreas", "# of zeroing requests", &zinfo->numZeroing);
    zinfo->rootStat->append(numZeroingStat);

    ProxyStat* numBaseIndexStat = new ProxyStat();
    numBaseIndexStat->init("numBaseIndexStat", "# of mem reads with base + index regs", &zinfo->numBaseIndex);
    zinfo->rootStat->append(numBaseIndexStat);

    ProxyStat* numBaseOnlyStat = new ProxyStat();
    numBaseOnlyStat->init("numBaseOnlyStat", "# of mem reads with base reg only", &zinfo->numBaseOnly);
    zinfo->rootStat->append(numBaseOnlyStat);

    ProxyStat* numIndexOnlyStat = new ProxyStat();
    numIndexOnlyStat->init("numIndexOnlyStat", "# of mem reads with index reg only", &zinfo->numIndexOnly);
    zinfo->rootStat->append(numIndexOnlyStat);

    ProxyStat* numNoRegStat = new ProxyStat();
    numNoRegStat->init("numNoRegStat", "# of mem reads with no base and no index reg", &zinfo->numNoRegMemRead);
    zinfo->rootStat->append(numNoRegStat);

    ProxyStat* numRegToRegStat = new ProxyStat();
    numRegToRegStat->init("numRegToRegStat", "# of reg to reg reads (pointer rewriting)", &zinfo->numRegToReg);
    zinfo->rootStat->append(numRegToRegStat);

    ProxyStat* numClearRegStat = new ProxyStat();
    numClearRegStat->init("numClearRegStat", "# of clear reg (pointer rewriting)", &zinfo->numClearReg);
    zinfo->rootStat->append(numClearRegStat);

    ProxyStat* numXchgMagicOpStat = new ProxyStat();
    numXchgMagicOpStat->init("numXchgMagicOpStat", "# of Xchg magic op", &zinfo->numXchgMagicOp);
    zinfo->rootStat->append(numXchgMagicOpStat);

    ProxyStat* zeroAllocBytesStat = new ProxyStat();
    zeroAllocBytesStat->init("zeroAllocBytes", "# of bytes zero alloc'ed", &zinfo->zeroAllocBytes);
    zinfo->rootStat->append(zeroAllocBytesStat);
}
...
void SimInit(const char* configFile, const char* outputDir, uint32_t shmid) {
    ...
    bool vmEnabled = config.get<bool>("sys.vm.enable", false);
    if (vmEnabled) {
        bool useLcp = config.get<bool>("sys.vm.useLcp", false);
        if (useLcp) {
            zinfo->vm = new LcpVirtualMemory(zinfo->rootStat, config);
        } else {
            zinfo->vm = new VirtualMemory(zinfo->rootStat, config);
        }
    }
    ...
    //reading cacheScrub-related variables
    zinfo->zeroAllocOpt = config.get<bool>("sim.withZeroAlloc", false);
    zinfo->zeroAllocLevel = config.get<uint32_t>("sim.zeroAllocLevel", 0);
    zinfo->cacheScrubInv = config.get<bool>("sim.withInvalidateDeadLines", false);
    zinfo->cacheScrubInvLevel = config.get<uint32_t>("sim.invalidateDeadLinesLevel", 0);

    //pointerRewrite
    zinfo->profilePointerRewrite = config.get<bool>("sim.profilePointerRewrite", false);
    zinfo->debugPointerRewrite = config.get<bool>("sim.debugPointerRewrite", false);
