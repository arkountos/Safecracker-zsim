...
class CC : public GlobAlloc {
    public:
        ...
        virtual void resetParents(const g_vector<MemObject*>& parents) {}
        ...
        //Cache Scrubbing
        virtual uint64_t passScrubInvReqToParent(MemReq& req) = 0; //flags get downgraded, may change req
        virtual uint64_t passZeroAllocReqToParent(MemReq& req) = 0; //flags get downgraded, may change req
};
...
class MESIBottomCC : public GlobAlloc {
    ...
    public:
        ...
        void resetParent(const g_vector<MemObject*>& _parents);
        ...
        void initStats(AggregateStat* parentStat) {
            ...
            profClZeroGETXMissIM.init("zeroAlloc_mGETXIM", "Zero Alloc - GETX I->M misses");
            profClZeroGETXMissSM.init("zeroAlloc_mGETXSM", "Zero Alloc - GETX S->M misses (upgrade misses)");
            profClZeroGETXHit.init("zeroAlloc_mGETXHit", "Zero Alloc - GETX Hits");
            profClInvINV.init("ClInvINV", "Invalidates caused by CLINV");
            ...
            parentStat->append(&profClZeroGETXMissIM);
            parentStat->append(&profClZeroGETXMissSM);
            parentStat->append(&profClZeroGETXHit);
            parentStat->append(&profClInvINV);
        ...
        uint64_t sendScrubInvReqToParent(MemReq& req);

        uint64_t sendZeroAllocReqToParent(MemReq& req);
        ...
        inline MESIState getStateOfLineId(uint32_t lineId) {
            return array[lineId];
        }
...
class MESITopCC : public GlobAlloc {
    ...
    public:
        ...
        uint64_t processEvictionWithNoWriteBack(Address wbLineAddr, uint32_t lineId, bool* reqWriteback, uint64_t cycle, uint32_t srcId);
...
class MESICC : public CC {
    ...
    public:
        ...
        void resetParents(const g_vector<MemObject*>& parents) {
            bcc->resetParent(parents);
        }
        ...
        uint64_t passScrubInvReqToParent(MemReq& req) {
            return bcc->sendScrubInvReqToParent(req);
        }

        uint64_t passZeroAllocReqToParent(MemReq& req) {
            return bcc->sendZeroAllocReqToParent(req);
        }
        ...
        uint64_t processEviction(const MemReq& triggerReq, Address wbLineAddr, int32_t lineId, uint64_t startCycle) {
            ...
            if (triggerReq.flags & MemReq::CACHE_SCRUB_1) {
                evCycle = tcc->processEvictionWithNoWriteBack(wbLineAddr, lineId, &lowerLevelWriteback, startCycle, triggerReq.srcId); //1. if needed, send invalidates/downgrades to lower level
        ...
        uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle, uint64_t* getDoneCycle = nullptr) {
            ...
            } else if ((req.flags & MemReq::ZERO_ALLOC_1) || (req.flags & MemReq::ZERO_ALLOC)) {
            ...
            if (lineId == -1 || (((req.type == PUTS) || (req.type == PUTX)) && !bcc->isValid(lineId))) { //can only be a non-inclusive wback
                ...
                MESIState originalState = bcc->getStateOfLineId(lineId);

                bool isZeroAllocTriggerLevel = (req.flags & MemReq::ZERO_ALLOC_1);
                assert(!IsZeroAlloc(req.flags) || isZeroAllocTriggerLevel);
                ...
                if (!isPrefetch) { //prefetches only touch bcc; the demand request from the core will pull the line to lower level
                    ...
                    if (isZeroAllocTriggerLevel) {
                        if ((originalState == S)) {
                            /*
                              if originalState is:
                              M / E / I: there should not be a need to change the state of the lower level cache
                              S: safer to invalidate all the copies in the lower level cache
                            */
                            assert(req.type == GETX);
                            tcc->sendInvalidates(req.lineAddr, lineId, INV, &lowerLevelWriteback, respCycle, req.srcId);
                            assert(*req.state == I);
                            // we would either invalidate all lower level copies, or not change it at all
                            lowerLevelWriteback = false;
                        }
                        //for ZERO_ALLOC: processAccess is only going to update the current directory and nothing else
                    ...
                    if (req.flags & MemReq::CACHE_SCRUB_1) {
                        ...
                        assert(*req.state == I);
                    } else if (req.flags & MemReq::ZERO_ALLOC_1) {
                        assert(!lowerLevelWriteback);
                        assert(*req.state == M);
                    }
...
/* FakeCC: Mostly-empty implementation of CC for caches *beyond the coherence
 * protocol* (e.g., non-inclusive and exclusive caches). Needed for repl policy
 * to classify lines correctly
 */
class FakeCC : public CC {
    private:
        MESIState* tag;
        g_vector<BaseCache*> children;
    public:
        FakeCC() : tag(NULL) {}
        void setTags(MESIState* _tag) {
            assert(!tag);
            tag = _tag;
        }

        //Initialization
        virtual void setParents(uint32_t childId, const g_vector<MemObject*>& parents, Network* network) {panic("!");}
        virtual void setChildren(const g_vector<BaseCache*>& _children, Network* network) { children = _children; }
        virtual void initStats(AggregateStat* cacheStat) {panic("!");}

        virtual uint32_t getNumChildren() const { return children.size(); }
        virtual BaseCache* getChild(uint32_t child) const { assert(child < children.size()); return children[child]; }

        //Access methods; see Cache for call sequence
        virtual bool startAccess(MemReq& req) {panic("!"); return false;}
        virtual bool shouldAllocate(const MemReq& req) {panic("!"); return false;}
        virtual uint64_t processEviction(const MemReq& triggerReq, Address wbLineAddr, int32_t lineId, uint64_t startCycle) {panic("!"); return 0;}
        virtual uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle, uint64_t* getDoneCycle) {panic("!"); return 0;}
        virtual void endAccess(const MemReq& req) {panic("!");}

        //Inv methods
        virtual void startInv() { panic("!");}
        virtual void releaseInvLock() { panic("!");}
        virtual uint64_t processInv(const InvReq& req, int32_t lineId, uint64_t startCycle) {panic("!"); return 0;}

        //Repl policy interface (THE ONLY THING WE IMPLEMENT)
        virtual uint32_t numSharers(uint32_t lineId) {return 0;}
        virtual bool isValid(uint32_t lineId) {return tag[lineId] != I;}
        virtual MESIState getState(uint32_t lineId) const {return tag[lineId];}

        virtual bool isSharer(uint32_t lineId, uint32_t childId) const {panic("Unimplemented"); return false;}

        //Misc
        virtual uint32_t getParentId(Address lineAddr) const {panic("!"); return 0;}

        //Cache Scrubbing
        virtual uint64_t passScrubInvReqToParent(MemReq& req) {panic("!"); return 0;}
        virtual uint64_t passZeroAllocReqToParent(MemReq& req) {panic("!"); return 0;}
};


/* Directory CC: Written from scratch, integrated tag
 * A directory does not store line data, so it differs from conventional caches in the following points:
 *  - It issues PUTX when MESI requires a writeback, without losing ownership (TODO: flag or new access type to get it to work OK)
 *  - In issues FWDs to get data; in the future, should optionally issue probes to upper level
 */
class MESIDirCC : public CC {
    private:
        struct Tag {
            //If state == I, invalid line
            //If state != I,
            //  If exclSharer >= 0, there is a single sharer with exclusive rights (E or M)
            //  Otherwise exclSharer == -1 and sharers is the sharer bit-vector
            //numSharers is always valid
            MESIState state; //can be I, S, or E; M only transient (we can't hold data, cannot be dirty!)
            int32_t exclSharer;
            std::bitset<MAX_CACHE_CHILDREN> sharers;
            uint32_t numSharers;

            Tag() : state(I), exclSharer(-1), numSharers(0) {}

            bool hasExclSharer() {
                return (exclSharer != -1);
            }

            uint32_t getExclSharer() {
                //assert(state != I);
                assert(exclSharer != -1);
                return (uint32_t)exclSharer;
            }

            void addSharer(uint32_t i, bool excl) {
                //assert(state != I);
                if (excl) {
                    assert(numSharers == 0);
                    assert(exclSharer == -1);
            ...
            void removeSharer(uint32_t i) {
                //assert(state != I);
                if (exclSharer != -1) {
                    assert((uint32_t)exclSharer == i);
                    assert(numSharers == 1);
                    exclSharer = -1;
                    numSharers = 0;
                } else {
            ...
            bool isSharer(uint32_t i) const {
               return (exclSharer != -1)? ((uint32_t)exclSharer == i) : sharers[i];
            }
        ...
        //Profiling:
        Counter profGETSParent, profGETSChild, profGETXParent, profGETXChild; //GETS/X requests, satisfied by lower/upper level
        Counter profPUTSParent, profPUTSFiltered, profPUTX; //clean/dirty PUT reqs
        Counter profINV, profINVX, profFWD; //from upper level
        Counter profEvUNUSED, profEvUSED, profEvINV; //evictions
        Counter profWbINVX, profWbEvINV; //writebacks to next level
        Counter profGETNextLevelLat, profGETNetLat; //critical-path next-level and network latencies lat
        Counter profINVCPLat, profINVXCPLat, profFWDCPLat; //critical-path inv/x/fwd latency for GET (demand) requests; includes network lat
        //NOTE: On requests with both up and down latencies that proceed in parallel, only the critical path latency is reported.
    ...
    public:
        //Initialization
        MESIDirCC(uint32_t _numLines, const g_string& name);
        void setParents(uint32_t childId, const g_vector<MemObject*>& parents, Network* network);
        void setChildren(const g_vector<BaseCache*>& children, Network* network);
        uint32_t getNumChildren() const;
        const BaseCache* getChild(uint32_t child) const;
        void initStats(AggregateStat* cacheStat);
        ...
        MESIState getState(uint32_t lineId) const;
        bool isReplica(uint32_t lineId) const {panic("Not implemented"); return false;}
    ...
    private:
        uint64_t sendInvalidates(Address lineAddr, uint32_t lineId, InvType type, bool* reqWriteback, uint64_t cycle, uint32_t srcId);
        uint64_t sendFwd(Address lineAddr, uint32_t lineId, uint64_t cycle, uint32_t srcId);
        uint64_t issueParentAccess(Address lineAddr, uint32_t lineId, AccessType type, uint64_t cycle, uint32_t srcId, uint32_t flags, uint32_t* netLat = nullptr);
        uint32_t getParentId(Address lineAddr) const;
        MemObject* getParent(Address lineAddr) const {panic("Not implemented");}
        //Cache Scrubbing
        virtual uint64_t passScrubInvReqToParent(MemReq& req) {panic("!"); return 0;}
        virtual uint64_t passZeroAllocReqToParent(MemReq& req) {panic("!"); return 0;}
