...
struct InstrFuncPtrs {  // NOLINT(whitespace)
    void (*loadPtr)(THREADID, ADDRINT, UINT32, ADDRINT, REG);
    void (*storePtr)(THREADID, ADDRINT, UINT32, ADDRINT, REG);
    ...
    void (*predLoadPtr)(THREADID, ADDRINT, UINT32, ADDRINT, BOOL, REG);
    void (*predStorePtr)(THREADID, ADDRINT, UINT32, ADDRINT, BOOL, REG);
    void (*afterStorePtr)(THREADID);
    uint64_t type;
    void (*regWritePtr)(THREADID, REG, ADDRINT, INT64, REG, BOOL);
    uint64_t pad[7];
...
class Core : public GlobAlloc {
    ...
    public:
        ...
        virtual Address getCurPC() const {
            panic("getCurPC() unimplemented for this core type (%s)", name.c_str());
            return 0;
        }
