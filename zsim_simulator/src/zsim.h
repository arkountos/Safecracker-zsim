...
struct GlobSimInfo {
    ...
    // want to avoid the case where the tid was not set (which might be read as 0),
    // and all tid 0's output is getting discarded
    // FIXME: add another boolean that tells you when these are all set
    uint32_t excessiveGCtid;
    uint32_t vmOpThreadtid;
    bool isInVmFF = false;
    // tids of threads that are currently in FF, and their ops will be ignored
    g_vector<bool>* fastForwardThreadTids;

    VirtualMemory* vm;

    uint64_t curRBP[MAX_THREADS];
    uint64_t curRSP[MAX_THREADS];

    bool zeroAllocOpt = false;
    uint32_t zeroAllocLevel = 0;
    bool cacheScrubInv = false;
    uint32_t cacheScrubInvLevel = 0;
    uint64_t zeroAllocBytes = 0;

    uint64_t numYoungGenGCs = 0;
    uint64_t numOldGenGCs = 0;
    uint64_t numDumpEvSt = 0;
    uint64_t numZeroing = 0;

    bool profilePointerRewrite = false;
    bool debugPointerRewrite = false;

    uint64_t numBaseIndex = 0;
    uint64_t numBaseOnly = 0;
    uint64_t numIndexOnly = 0;
    uint64_t numNoRegMemRead = 0;
    uint64_t numRegToReg = 0;
    uint64_t numClearReg = 0;
    uint64_t numXchgMagicOp = 0;
};
