...
class MD1Memory : public MemObject {
    private:
        ...
        // for cache scrubbing -- zero alloc
        Counter profZeroAllocReads;
        Counter profTotalZeroAllocRdLat;

        Counter profLoad;
        Counter profUpdates;
        Counter profClampedLoads;
        uint32_t curPhaseAccesses;
        VectorCounter profGETSByRanges, profGETXByRanges, profPUTSByRanges, profPUTXByRanges;
    ...
    public:
        ...
        void initStats(AggregateStat* parentStat) {
            ...
            profZeroAllocReads.init("zeroAllocRd", "Read requests due to Zero Alloc"); memStats->append(&profZeroAllocReads);
            profTotalZeroAllocRdLat.init("zeroAllocRdlat", "Total latency experienced by zero alloc read reqs"); memStats->append(&profTotalZeroAllocRdLat);
            ...
            memStats->append(&profGETSByRanges);
            memStats->append(&profGETXByRanges);
            memStats->append(&profPUTSByRanges);
            memStats->append(&profPUTXByRanges);
#endif
