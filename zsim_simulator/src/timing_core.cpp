...
void TimingCore::loadAndRecord(Address addr, MASize_t size, Address base, uint32_t baseReg) {
...
void TimingCore::loadSim(Address addr, MASize_t size, Address base, uint32_t baseReg) {
...
void TimingCore::storeAndRecord(Address addr, MASize_t size, Address base, uint32_t baseReg) {
    assert(maNum < MA_NUM_MAX);
    if (maNum == MA_NUM_MAX) panic("Memory access storage is out of bounds!");
    maAddr[maNum] = addr;
    maSize[maNum] = size;
...
void TimingCore::storeSim(Address addr, MASize_t size, Address base, uint32_t baseReg) {
    uint64_t startCycle = curCycle;
#ifdef MA_PROF_ENABLED
#   ifdef POINTER_TAGGING_ENABLED
    PointerTag_t tag = getPointerTag(base);
...
InstrFuncPtrs TimingCore::GetFuncPtrs() {
    ...
    return {LoadAndRecordFunc, StoreAndRecordFunc, BblAndRecordFunc, BranchFunc, PredLoadAndRecordFunc, PredStoreAndRecordFunc, AfterStoreFunc, FPTR_ANALYSIS, RegWriteFunc};
}

void TimingCore::LoadAndRecordFunc(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, REG baseReg) {
    assert(size < 256);
    static_cast<TimingCore*>(cores[tid])->loadAndRecord(addr, (MASize_t)size, base, baseReg);
}

void TimingCore::StoreAndRecordFunc(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, REG baseReg) {
    assert(size < 256);
    static_cast<TimingCore*>(cores[tid])->storeAndRecord(addr, (MASize_t)size, base, baseReg);
}
...
void TimingCore::PredLoadAndRecordFunc(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, BOOL pred, REG baseReg) {
    assert(size < 256);
    if (pred) static_cast<TimingCore*>(cores[tid])->loadAndRecord(addr, size, base, baseReg);
}

void TimingCore::PredStoreAndRecordFunc(THREADID tid, ADDRINT addr, UINT32 size, ADDRINT base, BOOL pred, REG baseReg) {
    assert(size < 256);
    if (pred) static_cast<TimingCore*>(cores[tid])->storeAndRecord(addr, size, base, baseReg);
}

void TimingCore::AfterStoreFunc(THREADID tid){
    return ;
}

void TimingCore::RegWriteFunc(THREADID tid, REG destReg, ADDRINT valueInBaseReg, INT64 offset, REG sourceReg, BOOL isClear) {
    assert_msg(false, "Unimplemented RegWriteFunc");
}
