...
Vagrant::Config.run do |config|
  ...
  config.vm.provision :shell, :inline => <<-SH
    ...
    # other packages
    apt-get -y install vim python-matplotlib
    ...
    echo 'kernel.shmmax = 1073741824' > /etc/sysctl.d/01-shmmax.conf
    echo 'kernel.yama.ptrace_scope = 0' > /etc/sysctl.d/02-ptrace_scope.conf
  SH
end
